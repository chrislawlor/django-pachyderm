...
class ArrayFieldTests(TransactionTestCase):
    def test_adding_rows(self):
        container = TextArrayContainer.objects.create(name='django', items=['framework', 'python'])
    ...
    def test_contains_array_element(self):
        TextArrayContainer.objects.create(name='django', items=['framework', 'python'])
        self.assertEqual(1, TextArrayContainer.objects.filter(items__contains='framework').count())
        self.assertEqual(0, TextArrayContainer.objects.filter(items__contains='php').count())
    
    def test_integer_type(self):
        container = IntegerArrayContainer.objects.create(name='container', items=[1,2,3,4,5])
        ...
        self.assertEqual(1, IntegerArrayContainer.objects.filter(items__contains=1).count())
    
    #def test_text_in_integer_array(self):
    #    with self.assertRaises(DatabaseError):
    #        container = IntegerArrayContainer.objects.create(name='container', items=['1', '2', '3'])
    #    self.assertEqual(0, IntegerArrayContainer.objects.all().count())
    
    @unittest.skipUnless(psycopg2_version_gte('2.4.5'), 'Array support for Inet type introduced in psycopg2 version 2.4.5')
    def test_inet_type(self):
        container = IPArrayContainer.objects.create(name='server1', items=[Inet('10.10.10.10'), Inet('10.10.10.11')])
